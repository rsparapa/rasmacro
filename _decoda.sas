%put NOTE: You have called the macro _DECODA, 2011-09-29;
%put NOTE: Copyright (c) 2001-2011 Rodney Sparapani;
%put;
...
/* _DECODA Documentation
    ...
    INFILE=                    CODA index file 
                               ...
                               if you are combining multiple runs, then this
    ...
    CONTENTS=0                 by default, no PROC CONTENTS
                               set to 1 to over-ride
    ...
    FORMAT=BEST6.              default format for statistics provided on
                               the histogram: Mean(SD), Q2(IQR)
                               either a single format for all the same
                               or a list of 4, in that order, for different
...
%macro _decoda(infile=REQUIRED, file=&infile, out=REQUIRED, alpha=0.025, autocorr=0, 
    bgr=&chains>1, burnin=, by=obs, chains=, contents=0, crosscorr=1, debug=, discrete=, drop=, 
    exp=, format=best6., graphics=%_version(8), gsfmode=replace, histogram=kernel, 
    marker=0.5, mu0=0, nlag=50, options=, pctldef=5, pctlpts=2.5 97.5, split=|, 
    symbol=i=join v=none r=1, stats=, tail=out, thin=1, trace=1, type=, var=&stats, 
    where=, width=4, log=, chain1=, chain2=, chain3=, chain4=, chain5=, chain6=, 
    chain7=, chain8=, chain9=, chain10=);
    ...
    %do i=1 %to &chains;
		...
		*/
        ...
        %if &chains=1 %then %let data1=&run1; /*data1=&out;*/
    ...
    proc transpose data=%if &chains=1 %then &data1; %else &out;
        out=&out(drop=_name_ sortedby=obs chain iter 
        %if %length(&exp)=0 & %length(&log)=0 %then index=(ci=(chain iter));
        );
        by obs chain iter; %*weight;
    	where %if %length(&burnin) %then iter>&burnin;;
        var value;
        id var;
        idlabel label;
    run;
    ...
    %if %length(&var) %then %_debugs(data=&out, var=&var, alpha=&alpha, 
        autocorr=&autocorr, bgr=&bgr, by=&by, chains=&chains, crosscorr=&crosscorr, 
        debug=&debug, discrete=&discrete, format=&format, graphics=&graphics, 
        gsfmode=&gsfmode, histogram=&histogram, marker=&marker, mu0=&mu0, nlag=&nlag, 
        options=&options, pctldef=&pctldef, pctlpts=&pctlpts, symbol=&symbol, 
        trace=&trace, type=&type, width=&width);
